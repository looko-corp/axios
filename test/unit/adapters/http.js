...
function generateReadableStream(
  length = 1024 * 1024,
  chunkSize = 10 * 1024,
  sleep = 50
) {
...
describe("supports http with nodejs", function () {
  ...
  describe("streams", function () {
    ...
    it("should destroy the response stream with an error on request stream destroying", async function () {
      ...
      const { data } = await axios.post(LOCAL_SERVER_URL, stream, {
        responseType: "stream",
      });
          ...
          .listen(4444, function () {
            axios
              ...
              .then(function (res) {
                ...
                assert.strictEqual(
                  res.data.files.fileStream.mimetype,
                  "image/png"
                );
                assert.strictEqual(
                  res.data.files.fileStream.originalFilename,
                  "axios.png"
                );
